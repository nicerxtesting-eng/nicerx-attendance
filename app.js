  ...
  if (state.sheetUrl) {
    try {
      if (method === 'GET') {
        ...
        const response = await fetch(`${state.sheetUrl}?${queryParams}`);
      ...
      showNotification(`Sheet API failed: ${err.message}. Offline sandbox used.`, 'error');
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (action === 'login') {
        ...
        const match = users.find(u => u.email.toLowerCase() === payload.email.toLowerCase() && u.passwordHash === pwdHash);
...
function setupEventListeners() {
  ...
  if (DOM.adminExportBtn) {
    DOM.adminExportBtn.addEventListener('click', () => {
      // Populate select dropdown with agents list
      ...
      // Default dates (current month start to today in EST)
      const todayEST = getESTTime();
      const firstDay = new Date(todayEST.getFullYear(), todayEST.getMonth(), 1);
...
function restoreShiftStateAndTimers() {
  const todayStr = getTodayStringEST(); // Check current date in EST
  const userAttendance = state.dashboardData.attendance.filter(
    a => a.email.toLowerCase() === state.currentUser.email.toLowerCase() && a.date === todayStr
  );
  
  const userBreaks = state.dashboardData.breaks.filter(
    b => b.email.toLowerCase() === state.currentUser.email.toLowerCase() && b.date === todayStr
  );

  const activeShift = userAttendance.find(a => a.punchOut === '');
  
  // Calculate break categories from logs today
  calculateCompletedBreaksToday(userBreaks);
  
  if (activeShift) {
    state.punchInTime = new Date(activeShift.punchIn);
    state.totalBreakMs = userBreaks
      .filter(b => b.breakEnd !== '')
      .reduce((sum, b) => sum + (new Date(b.breakEnd) - new Date(b.breakStart)), 0);
      
    const activeBreak = userBreaks.find(b => b.breakEnd === '');
    if (activeBreak) {
...
function handleExportReportAction() {
  ...
  // Filter by date range (dates are stored as YYYY-MM-DD)
  logs = logs.filter(l => {
  ...
  link.setAttribute("download", `NiceRx_Attendance_Report_${agentFilename}_${startDateStr}_to_${endDateStr}.csv`);
